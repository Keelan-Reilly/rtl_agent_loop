...
6. `top_level.sv` wires buffers, control, interconnect, and clusters into a synthesis-friendly top module and commits one output lane per writeback cycle.
...
- `ARRAY_M` and `ARRAY_N` scale both logical lane count and output count:
  - more lanes instantiate more clusters
  - writeback latency scales with `ARRAY_M * ARRAY_N`
